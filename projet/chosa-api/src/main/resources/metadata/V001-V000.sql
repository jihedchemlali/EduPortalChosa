...
ALTER TABLE `APP_ACTUALITY_CHILD` ADD `USER_ID_FK` bigint(20) NOT NULL AFTER `CHILD_ID_FK`;
ALTER TABLE `APP_TRAINING_CENTER` ADD `FOUNDATIONDAY` DATE NULL AFTER `PHONE`;
UPDATE `APP_TRAINING_CENTER` SET `FOUNDATIONDAY` = DATE(NOW()) WHERE `APP_TRAINING_CENTER`.`FOUNDATIONDAY` is NULL
ALTER TABLE `APP_ACTIVITY` CHANGE `AGE` `AGE` INT(2) NULL;
ALTER TABLE `APP_ACTIVITY` ADD `FILE_ID_FK` BIGINT(20) NULL AFTER `DATE_PUBLICATION`;
ALTER TABLE APP_FILE DROP TYPE;
ALTER TABLE `APP_ACTUALITY_CHILD` ADD `LONGITUDE` VARCHAR(12) NULL DEFAULT NULL AFTER `TYPE`;
ALTER TABLE `APP_ACTUALITY_CHILD` ADD `LATITUDE` VARCHAR(12) NULL DEFAULT NULL AFTER `LONGITUDE`;
